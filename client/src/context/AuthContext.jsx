...
export const AuthProvider = ({ children }) => {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem('spp_user') || sessionStorage.getItem('sih_user');
    if (saved) {
      try {
  ...
  const [role, setRole] = useState(() => {
    return localStorage.getItem('spp_role') || sessionStorage.getItem('sih_role') || null;
  });
  ...
  const login = (selectedRole, identifier = '', password = '') => {
    ...
    // If identifier is passed as a full user object:
    if (typeof identifier === 'object' && identifier !== null) {
      const customUser = identifier;
      _saveSession(targetRole, customUser);
  ...
  const _saveSession = (r, u) => {
    ...
    sessionStorage.setItem('sih_role', r);
    sessionStorage.setItem('sih_user', jsonU);
  };
