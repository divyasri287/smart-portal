...
import { ShieldCheck, Tractor, ClipboardCheck, Building2, Landmark, ArrowLeft, Send } from 'lucide-react';
...
export const Login = () => {
  ...
  const handleFarmerLogin = (e) => {
    ...
    setTimeout(() => {
      ...
      if (res.success) {
        ...
        setErrorMsg(res.message || 'Login failed. Please verify your mobile number or register.');
      }
    }, 300);
  };
  ...
  const handleStaffLogin = (e) => {
    ...
    setErrorMsg('');
    if (!idInput.trim()) {
      setErrorMsg(`Please enter your ${activeRole.toUpperCase()} ID.`);
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (res.success) {
        ...
        switch (activeRole) {
          case ROLES.OFFICER: navigate('/officer/dashboard'); break;
          case ROLES.MANAGER: navigate('/manager/dashboard'); break;
          case ROLES.ADMIN:   navigate('/admin/dashboard');   break;
          default:            navigate('/select-role');
        }
      } else {
        setErrorMsg(res.message || `Invalid ${activeRole.toUpperCase()} ID or Password.`);
      }
    }, 300);
  };
    ...
    <div className="space-y-5 max-w-lg mx-auto select-none cursor-default">
      ...
      {activeRole === ROLES.FARMER && (
        <div className="bg-emerald-800 text-white p-5 rounded-2xl shadow-sm border border-emerald-700 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-emerald-200 uppercase tracking-wider block">Farmer Access</span>
            <h1 className="text-2xl font-black tracking-tight">Farmer Login</h1>
            <p className="text-xs text-emerald-100 mt-0.5">Login using your Mobile Number</p>
          </div>
      ...
      {activeRole === ROLES.OFFICER && (
        <div className="text-white p-5 rounded-2xl shadow-sm border border-emerald-700 flex items-center justify-between" style={{ backgroundColor: '#065F46' }}>
          <div>
            <span className="text-xs font-semibold text-emerald-200 uppercase tracking-wider block">Procurement Officer</span>
      ...
      {activeRole === ROLES.MANAGER && (
        <div className="bg-emerald-900 text-white p-5 rounded-2xl shadow-sm border border-emerald-800 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-emerald-200 uppercase tracking-wider block">Centre Management</span>
      ...
      {activeRole === ROLES.ADMIN && (
        <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-sm border border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-300 uppercase tracking-wider block">State &amp; National Directorate</span>
      ...
      {activeRole === ROLES.FARMER && (
        <form onSubmit={handleFarmerLogin} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-2 py-1">
            ...
            />
            <label htmlFor="remember-farmer" className="text-xs text-slate-600 font-medium cursor-pointer">
              Remember login on this browser session
            </label>
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs">
            ...
            <button type="button" onClick={() => navigate('/select-role')} className="text-slate-500 hover:text-slate-800 font-medium cursor-pointer">
      ...
      {/* ── OFFICER / MANAGER / ADMIN LOGIN ── */}
      {(activeRole === ROLES.OFFICER || activeRole === ROLES.MANAGER || activeRole === ROLES.ADMIN) && (
        <form onSubmit={handleStaffLogin} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          <Input
            ...
            placeholder={`Enter ${activeRole === ROLES.OFFICER ? 'Officer' : activeRole === ROLES.MANAGER ? 'Manager' : 'Admin'} ID`}
            value={idInput}
